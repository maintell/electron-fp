...
void PrintViewManagerElectron::RenderDocument() {
  ...
  // The renderer defers printing while its frame is loading, and a page
  // window.print() in that window would take over its print state.
  if (web_contents()->IsLoading()) {
    job_->waiting_for_load = true;
    return;
  }
...
void PrintViewManagerElectron::Finish(bool success, std::string_view reason) {
  ...
  if (job.print_job) {
    ...
    // A fully rendered document is left to PrintJobManager to finish spooling.
