...
// --- Tab Manager (two layers: Group = profile browser, Tab = view in group) ---
// Group shares ONE partition/session (cookie/storage/UA/TLS/H2); tabs in the
// group are views on it. Groups are isolated from each other.
/** @type {Map<string, {id: string, groupId: string, view: BrowserView, url: string, title: string}>} */
const tabs = new Map();
/** @type {Map<string, {id: string, profileId: string, profileName: string, userAgent: string, blink: object|null, tls: object|null, h2: object|null, tabIds: string[], activeTabId: string|null}>} */
...
/**
 * Mount only the active tab's view. All groups/tabs share the window but a
 * single native view is attached at a time.
 */
 ...
 * partition afterwards does pick it up. Since both createGroupView() and
 * recreateGroupViews() construct a fresh view, calling this first is what makes
 * the UA take effect.
 *
 * Passing '' reverts to the native UA (measured: setUserAgent('') falls back
 * to Chromium's own UA rather than sending a blank one).
 *
 * UA is per-partition, and every group owns a unique partition, so
 * per-group UA isolation comes for free.
 */
...
/**
 * Attach page-event forwarding for a group view. Shared by createGroupView()
 * and recreateGroupViews() so the two cannot drift.
 */
function attachGroupViewEvents(view, tabId, groupId) {
  view.webContents.on('page-title-updated', (e, title) => {
    const tab = tabs.get(tabId);
    if (tab) {
      tab.title = title;
      mainWindow?.webContents.send('tab:title-updated', { tabId, groupId, title });
    }
  });
  ...
  view.webContents.on('did-navigate', (e, url) => {
    ...
    if (tab) {
      tab.url = url;
      mainWindow?.webContents.send('tab:navigated', { tabId, groupId, url });
    }
  });
...
/**
 * Create a BrowserView for a tab inside a group.
 * The partition is shared per GROUP (`fp-group-{groupId}`) so cookies/storage/
 * UA/TLS/H2 are shared group-wide and isolated group-to-group. Every view in
 * the group is built with the same blink fingerprint.
 *
 * H2/TLS are applied only for the group's FIRST tab: H2 freezes at the first
 * fromPartition() for the name, TLS must precede the first socket. Later tabs
 * reuse the live session (UA is re-set, harmlessly, so a fresh view picks it
 * up — see applyTabUserAgent()).
 */
function createGroupView(group, tabId) {
  // Unique partition per GROUP for cookie/session/storage isolation
  const partition = `fp-group-${group.id}`;
  const isFirst = group.tabIds.length === 0;
  ...
  if (isFirst) {
    ...
    applyTabUserAgent(partition, group.userAgent, group.h2);
    if (group.tls && Object.keys(group.tls).length) applyTabTLSConfig(partition, group.tls);
  } else {
    // No H2 argument here: the partition already exists, so passing one is a
    // no-op. It is set once, at group creation.
    applyTabUserAgent(partition, group.userAgent);
  }
  ...
  const view = new BrowserView({
    webPreferences: {
      ...
      ...(group.blink ? { fingerprint: group.blink } : {})
    }
  });

  attachGroupViewEvents(view, tabId, group.id);
  return view;
}

/**
 * Recreate ALL views of a group with a new fingerprint config.
 * Reuses the SAME partition so cookies/storage persist; each renderer
 * process is respawned so the new --fingerprint-config takes effect.
 * Each tab keeps its own url. Remounts the active view afterwards.
 */
function recreateGroupViews(groupId, { blink, userAgent, tls, h2 }) {
  ...
  // which already happened for the group's first tab. Passing a profile here
  // would be silently discarded (measured), so instead of pretending, compare
  // and report: if the caller asked for something different from what the
  // partition was built with, say so. A control that looks applied but is not
  // is worse than a documented limitation - it is the "claims a fingerprint it
  // does not produce" failure the 50-electron-glue patch calls out.
  const wantH2 = h2 && Object.keys(h2).length ? h2 : null;
  const haveH2 = group.h2 && Object.keys(group.h2).length ? group.h2 : null;
  group.h2Refused = null;
  if (wantH2 && JSON.stringify(wantH2) !== JSON.stringify(haveH2)) {
    group.h2Refused = 'HTTP/2 settings are fixed when the tab is created; ' +
      'close and reopen the tab to change them.';
    console.warn('[fp] ' + group.h2Refused);
  }

  // UA must be set before the views are constructed – see applyTabUserAgent().
  // No H2 argument here: the partition already exists, so passing one is a
  ...
  // socket, so it must be on the session before the views' first request.
  // Persisted on the group so the self-test and the UI can report what is live.
  group.tls = tls || null;
  const tlsRes = applyTabTLSConfig(partition, group.tls);
  // Recorded on the group rather than returned, because callers already have
  // the group and changing the return shape would touch every caller.
  group.tlsRefused = tlsRes.ok ? null : (tlsRes.error || 'TLS config was refused');
  group.blink = blink || null;
  ...
  for (const tid of [...group.tabIds]) {
    ...
    attachGroupViewEvents(view, tid, group.id);
    tab.view = view;
...
function addGroup(profileId = 'default') {
  const profiles = loadProfiles();
  const profile = profiles.profiles.find(p => p.id === profileId) || profiles.profiles[0];
  // Split the profile into its THREE delivery planes. This is the group-CREATION
  ...
  const split = profile.fingerprint ? fpSplitConfig(profile.fingerprint) : null;
  ...
  // without this the drift check in recreateGroupViews() has nothing to compare
  ...
  const group = {
    ...
    blink: split ? split.fingerprint : null,
    tls: split ? split.tls : null,
    h2: (split && split.h2) ? split.h2 : null,
    tabIds: [],
    activeTabId: null,
    h2Refused: null,
    tlsRefused: null,
  ...
  mainWindow?.webContents.send('group:created', { groupId, profileId: group.profileId, profileName: group.profileName });
  const tabId = addTab(groupId);
  activeGroupId = groupId;
  mainWindow?.webContents.send('group:activated', { groupId });
  return groupId;
}
...
function closeGroup(groupId) {
  ...
  for (const tid of [...group.tabIds]) {
    ...
    if (tab) {
      ...
      tabs.delete(tid);
      mainWindow?.webContents.send('tab:closed', { tabId: tid, groupId: gid });
    }
  }

  groups.delete(gid);
  mainWindow?.webContents.send('group:closed', { groupId: gid });
  ...
  if (activeGroupId === gid) {
    ...
    if (remaining.length > 0) {
      activateGroup(remaining[remaining.length - 1]);
    } else {
      showActiveView();
    }
  }
...
function activateGroup(groupId) {
  ...
  activeGroupId = groupId;
  if (group.activeTabId && tabs.has(group.activeTabId)) {
    activeTabId = group.activeTabId;
  } else if (group.tabIds.length > 0) {
    activeTabId = group.tabIds[group.tabIds.length - 1];
    group.activeTabId = activeTabId;
  } else {
    activeTabId = null;
  }
  showActiveView();
  ...
  const tab = tabs.get(activeTabId);
  if (tab) {
    mainWindow?.webContents.send('tab:activated', { tabId: tab.id, groupId, profileId: group.profileId, profileName: group.profileName, url: tab.url, title: tab.title });
  }
}

function addTab(groupId, url) {
  const gid = groupId || activeGroupId;
  const group = groups.get(gid);
  if (!group) return null;
  const tabId = createTabId();
  const view = createGroupView(group, tabId);
  tabs.set(tabId, { id: tabId, groupId: gid, view, url: 'about:blank', title: 'New Tab' });
  group.tabIds.push(tabId);
  if (typeof url === 'string' && url) {
    tabs.get(tabId).url = url;
    try { view.webContents.loadURL(tabs.get(tabId).url); } catch {}
  }

  const tab = tabs.get(tabId);
  mainWindow?.webContents.send('tab:created', { tabId, groupId: gid, profileId: group.profileId, profileName: group.profileName, url: tab.url, title: tab.title });
  activateTab(tabId);
  return tabId;
}
...
function closeTab(tabId) {
  ...
  if (group) {
    ...
    if (group.activeTabId === tabId) {
      group.activeTabId = group.tabIds.length ? group.tabIds[group.tabIds.length - 1] : null;
    }
  }
  mainWindow?.webContents.send('tab:closed', { tabId, groupId: tab.groupId });

  // Activate another tab if this was active
  if (activeTabId === tabId) {
    activeTabId = null;
    if (group && group.tabIds.length > 0) {
      activateTab(group.tabIds[group.tabIds.length - 1]);
    } else {
      // Group is empty: fall back to the last tab of the last group, if any.
...
function activateTab(tabId) {
  ...
  // Cross-group activation moves the active group as well.
  activeGroupId = group.id;
  activeTabId = tabId;
  group.activeTabId = tabId;
  showActiveView();

  mainWindow?.webContents.send('tab:activated', { tabId, groupId: group.id, profileId: group.profileId, profileName: group.profileName, url: tab.url, title: tab.title });
}
...
function createMainWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    const gid = addGroup('default');
    const g = groups.get(gid);
    for (const site of FP_AUDIT_SITES) {
      const id = addTab(gid);
      if (id) navigateTab(id, site);
    }
    if (g && g.tabIds.length) activateTab(g.tabIds[0]);
  });
}
...
function setupIPC() {
  ...
  ipcMain.handle('group:create', (e, profileId) => addGroup(profileId || 'default'));
  ipcMain.handle('group:close', (e, groupId) => closeGroup(groupId));
  ipcMain.handle('group:activate', (e, groupId) => activateGroup(groupId));
  ipcMain.handle('group:list', () => {
    const list = [];
    for (const [id, group] of groups) {
      list.push({ id, profileId: group.profileId, profileName: group.profileName, tabIds: [...group.tabIds], activeTabId: group.activeTabId, isActive: id === activeGroupId });
    }
    return list;
  });
  ipcMain.handle('group:get-active', () => activeGroupId);
  ipcMain.handle('group:set-profile', (e, { groupId, config, userAgent }) => {
    return applyGroupProfile(groupId || activeGroupId, config, userAgent);
  });

  // Tab operations
  ipcMain.handle('tab:create', (e, arg) => {
    // Compat: string = legacy profileId OR a groupId (renderer passes
    // activeGroupId through createTab for "new sub-tab in this group").
    // Missing groupId falls back to activeGroupId.
    if (typeof arg === 'string') {
      if (arg && groups.has(arg)) return addTab(arg);
      const pid = arg || 'default';
      const ag = groups.get(activeGroupId);
      if (ag && ag.profileId === pid) return addTab(activeGroupId);
      const gid = addGroup(pid);
      const g = groups.get(gid);
      return g ? g.activeTabId : null;
    }
    if (arg && typeof arg === 'object') {
      if (arg.groupId && groups.has(arg.groupId)) return addTab(arg.groupId, arg.url);
      if (arg.profileId) {
        const ag = groups.get(activeGroupId);
        if (ag && ag.profileId === arg.profileId) return addTab(activeGroupId, arg.url);
        const gid = addGroup(arg.profileId);
        const g = groups.get(gid);
        return g ? g.activeTabId : null;
      }
      return addTab(activeGroupId, arg.url);
    }
    return addTab(activeGroupId);
  });
  ...
  ipcMain.handle('tab:list', () => {
    ...
    for (const [id, tab] of tabs) {
      const group = groups.get(tab.groupId);
      list.push({ id, groupId: tab.groupId, profileId: group ? group.profileId : null, profileName: group ? group.profileName : null, url: tab.url, title: tab.title, isActive: id === activeTabId });
    }
    return list;
  });
  ipcMain.handle('tab:get-active', () => ({ groupId: activeGroupId, tabId: activeTabId }));

  // Shared group-profile apply: split into the three delivery planes,
  // recreate the group's views on the same partition, broadcast, report drift.
  // Used by group:set-profile directly; tab:set-fingerprint is its alias for
  // the tab's own group.
  function applyGroupProfile(gid, config, userAgent) {
    const group = groups.get(gid);
    ...
    try {
      ...
      // Recreate the renderers (same partition) so the new fingerprint config
      // is injected via --fingerprint-config at renderer startup, the new
      // UA is picked up by the fresh views, and the TLS config is on the
      // session before those views' first socket.
      // HTTP/2 is handed over only so recreateGroupViews can detect and report
      // a change it cannot apply - the plane itself is frozen at creation.
      recreateGroupViews(gid, { blink: apply, userAgent: ua, tls: tlsApply, h2: h2Apply });
      group.profileId = config ? 'custom' : 'default';
      group.profileName = config ? 'Custom' : 'Default';
      mainWindow?.webContents.send('group:profile-changed', { groupId: gid, profileId: group.profileId, profileName: group.profileName });
      for (const tid of group.tabIds) {
        mainWindow?.webContents.send('tab:profile-changed', { tabId: tid, groupId: gid, profileId: group.profileId, profileName: group.profileName });
      }
      ...
      if (tlsApply && Object.keys(tlsApply).length && group.tlsRefused) {
        return { ok: true, tlsError: group.tlsRefused };
      }
      // Same honesty for HTTP/2: the plane cannot be changed on a live group, so
      // say so rather than returning success for a setting that did not move.
      if (group.h2Refused) {
        return { ok: true, h2Error: group.h2Refused };
      }
      return true;
    } catch { return false; }
  }

  // Tab fingerprint operations (group semantics: read/apply via the tab's group)
  ...
  ipcMain.handle('tab:set-fingerprint', (e, { tabId, config, userAgent }) => {
    ...
    const tab = tabs.get(tid);
    if (!tab) return false;
    return applyGroupProfile(tab.groupId, config, userAgent);
  });
  ...
  ipcMain.handle('tab:get-ua', (e, tabId) => {
    ...
    const group = groups.get(tab.groupId);
    try {
      return session.fromPartition(`fp-group-${tab.groupId}`).getUserAgent() || '';
    } catch { return (group ? group.userAgent : '') || ''; }
  });
  ...
  ipcMain.handle('tab:set-ua', (e, { tabId, userAgent }) => {
    ...
    if (!group) return false;
    try {
      // Same partition as the group, so cookies/storage survive the respawn.
      const ua = fpNormalizeUserAgent(userAgent);
      // Preserve the live blink/TLS config. Omitting TLS here would silently
      // reset the group to the native ClientHello on every UA change, because
      // recreateGroupViews() applies whatever it is handed - and "not handed"
      // is indistinguishable from "deliberately cleared".
      // group.h2 is passed so a mismatch is reported; it cannot actually change
      // here (the partition was built at creation), and passing it is what
      // makes that visible instead of silent.
      let blink = null;
      try { blink = tab.view.webContents.getFingerprintConfig?.() || group.blink; } catch { blink = group.blink; }
      recreateGroupViews(group.id, { blink, userAgent: ua, tls: group.tls || null, h2: group.h2 || null });
      mainWindow?.webContents.send('group:profile-changed', {
        groupId: group.id, profileId: group.profileId, profileName: group.profileName
      });
      mainWindow?.webContents.send('tab:profile-changed', {
        tabId: tid, groupId: group.id, profileId: group.profileId, profileName: group.profileName
      });
      return true;
    } catch { return false; }
  });
